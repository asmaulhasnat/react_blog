...
const history=useHistory();
 ...
 const handleSubmit =(e) =>{
 	e.preventDefault();
 	axios.post(`${env.API_URL}/users/signup`,Formdata)
      .then(res => {
...
<div className="card bg-secondary mt-3 ml-5 mr-5">
		    ...
		    {error != '' ? (<span className='bg-danger'>{error}</span>):''}
		 	...
		 	<div className="mb-3">
			  ...
			  <input type="name" className="form-control" id="name" placeholder="Enter your name" onChange={(e)=>{handleChange(e.target.value,'name')}}/>
			</div>
		 	<div className="mb-3">
